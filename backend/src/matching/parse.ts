  ...
  for (const code of storeCodes) {
    ...
    queries[code] =
      typeof query === "string" && query.trim()
        ? query.trim()
        : fallback.queries[code];
  }
  const sizeHint =
    raw.sizeHint &&
    typeof raw.sizeHint.amount === "number" &&
    raw.sizeHint.amount > 0 &&
    UNITS.includes(raw.sizeHint.unit as Unit)
      ? { amount: raw.sizeHint.amount, unit: raw.sizeHint.unit }
      : null;
  return {
    canonicalName:
      typeof raw.canonicalName === "string" && raw.canonicalName.trim()
        ? raw.canonicalName.trim().toLowerCase()
    ...
    queries,
    fallbackQuery:
      typeof raw.fallbackQuery === "string" && raw.fallbackQuery.trim()
        ? raw.fallbackQuery.trim()
        : null,
...
export async function parseIdea(
  ...
  const key = await sha256(
    `${PARSE_PROMPT_VERSION}|${appLanguage()}|${normalizeText(text)}|${storeCodes.join(",")}`,
  );
  const raw = await cachedChatJson<Partial<ParsedIdea>>(
    ...
    [
      {
        role: "system",
        content: SYSTEM.replaceAll("LANGUAGE", appLanguageName()),
      },
      {
        role: "user",
        content: `Store codes: ${storeCodes.join(", ")}\nIdea: ${text}`,
      },
    ],
    { maxTokens: 900 },
  );
...
export async function splitShoppingText(
  text: string,
): Promise<Array<{ text: string; quantity: number }>> {
  const fallback = text
    .split(/[\n,;]+|\s+\b(?:and|en|y)\b\s+/gi)
    .map((part) => part.trim())
    .filter(Boolean)
    ...
    .map((part) => {
      ...
      return match
        ? { text: match[2], quantity: Number(match[1]) }
...
export async function alternativeQuery(
  text: string,
  parsed: ParsedIdea,
  store: string,
  ...
  const raw = await chatJson<{ queries?: unknown; query?: string }>(
    [
      {
        role: "system",
        content: `The user shops at a Dutch supermarket webshop and did not find what they meant. First understand WHAT the item is (its category and role: e.g. "krulsla melange" is a bag of mixed salad leaves). Then propose up to 4 short Dutch search queries for ALTERNATIVES a shopper would accept instead: the broader category ("gemengde sla"), sibling products ("rucola", "ijsbergsla", "veldsla"), and the same thing under another name. Never repeat or merely reword the earlier queries. If a product the user already picked at ANOTHER store is given, aim for its closest equivalent here: same product type, same kind of pack (pieces/stuks vs grams vs millilitres) and a similar size, e.g. "eieren 10 stuks" or "halfvolle melk 1 liter". Return ONLY {"queries": ["...", ...]}.`,
      },
      {
        role: "user",
        content: `Idea: ${text}\nCanonical: ${parsed.canonicalName}\nAttributes: ${parsed.attributes.join(", ") || "none"}\nStore: ${store}\nQueries already tried (do not repeat): ${triedQueries.join(" | ")}\nProducts the user rejected or that were not it: ${rejectedTitles.join(" | ") || "none"}${reference ? `\nAlready picked at another store (find the equivalent): ${reference}` : ""}`,
      },
    ],
    { maxTokens: 400 },
  );
