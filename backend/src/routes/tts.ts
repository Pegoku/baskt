...
import { speechModels, speechSamples, speechChoice, synthesize } from "@/ai/speak";

export const tts = new Hono();
tts.get("/voices", (c) => c.json({ configured: !!process.env.REPLICATE_API_TOKEN, models: speechModels }));
tts.post("/", async (c) => {
